...
<div class="blockreassurance">
    <div class="container">
        ...
        <div class="rcerow row displayflex">
            <div class="slicksliderrce ">
                {foreach from=$blocks item=$block key=$key name=blocks}
                    ...
                    {if $idxCol > ($numCols - $numColsRemaining_sm)}
                        ...
                        {/if}
                    {/if}
                    <div class="rce_item rce_item_{$key} col-md-3 col-sm-6 col-xs-12">
                        <div class="rce_item_cover"
                            ...
                            <div class="block_title" style="color:{$textColor}">{$block['title']}</div>
                            <div class="block_desc" style="color:{$textColor};">{$block['description'] nofilter}</div>
                        </div>
                    </div>
                {/foreach}
            </div>
            <div class="avisverifies">
               <!-- <img src="{$urls.img_url}avisverifies.png" alt=""> -->
				{hook h='GlobalnoteNetreviews'}
            </div>
        </div>
    </div>
</div>
