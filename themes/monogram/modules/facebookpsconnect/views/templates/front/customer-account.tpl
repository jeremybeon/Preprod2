...
{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">{l s='My account'  mod='facebookpsconnect'}</a><span class="navigation-pipe">{$navigationPipe|escape:'htmlall':'UTF-8'}</span><span class="navigation_page">{l s='Connection with Social Networks'  mod='facebookpsconnect'}</span>{/capture}
				...
				<tr>
					...
					<td class="center">
						...
						{if !empty($aLinkedSocial)}
							...
							{else}
								<a class="btn btn-default float-right disabled"><i class="fa fa-disable"></i> {l s='No link' mod='facebookpsconnect'}</a>
							{/if}
						{else}
							<a class="btn btn-default float-right disabled">{l s='No link' mod='facebookpsconnect'}</a>
						{/if}
					</td>
