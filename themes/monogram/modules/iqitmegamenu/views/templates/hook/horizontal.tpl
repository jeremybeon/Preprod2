...
*}
	...
	{if $tab.url_type == 2}<a role="button" class="cbp-empty-mlink">{else}<a href="{$tab.url}" {if $tab.new_window}target="_blank"{/if}>{/if}
								...
								{if !$tab.icon_type && !empty($tab.icon)} <img src="{$tab.icon}" alt="{$tab.title}" class="cbp-mainlink-iicon" />{/if}{if !$tab.active_label}{$tab.title|replace:'/n':'<br />'}{/if}{if $tab.submenu_type} <i class="fa fa-angle-down cbp-submenu-aindicator hidden-sm-down"></i>{/if}</span>
						...
						{if $tab.submenu_type} <i class="fa fa-angle-right cbp-submenu-aindicator hidden-md-up"></i>{/if}

							{if $tab.submenu_type && !empty($tab.submenu_content)}
							<div class="cbp-hrsub col-xs-{$tab.submenu_width}">
								<div class="cbp-triangle-container hidden-md-up">
