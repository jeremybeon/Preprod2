...
{block name='head_seo'}
    ...
    {block name='head_hreflang'}
        {assign "languages" Language::getLanguages(true)}
    {foreach from=$urls_redirect item=pageUrl key=code}
    {foreach $languages "langue"}
    {if $code == $langue['iso_code']}<link rel="alternate" href="{$pageUrl}" hreflang="{$langue['locale']}">
    {/if}
    {/foreach}
    {/foreach}
    {/block}
        ...
        {foreach from=$Pagination.PremieresPages key=key_page item=value_page}
            ...
            {else}
                {if $Pagination.PageCourante == ($key_page-1)}
                    <link rel="next"
                        href="{PrestaBlogUrl categorie=$prestablog_categorie_link_rewrite start=$value_page p=$key_page c=$prestablog_categorie m=$prestablog_month y=$prestablog_year}{$prestablog_search_query|escape:'htmlall':'UTF-8'}">
                {else if $Pagination.PageCourante == $key_page}
                    <link rel="prev"
                        href="{PrestaBlogUrl categorie=$prestablog_categorie_link_rewrite c=$prestablog_categorie m=$prestablog_month y=$prestablog_year}{$prestablog_search_query|escape:'htmlall':'UTF-8'}">
