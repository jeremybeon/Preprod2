    ...
    <ul class="carousel-inner" role="listbox" aria-label="{l s='Carousel container' d='Shop.Theme.Global'}">
      {foreach from=$homeslider.slides item=slide name='homeslider'}
        <li class="carousel-item {if $smarty.foreach.homeslider.first}active{/if}" role="option" aria-hidden="{if $smarty.foreach.homeslider.first}false{else}true{/if}">
          {*<a href="{$slide.url}">*}
            <figure>
              <img class="slide_image" src="{$slide.image_url}" alt="{$slide.legend|escape}" loading="lazy">
              {if $slide.title || $slide.description}
                <figcaption class="caption hidden-sm-down">
                  <h2 class="display-1 text-uppercase">{$slide.title}</h2>
                  <div class="caption-description">{$slide.description nofilter}</div>
                </figcaption>
                <figcaption class="caption hidden-md-up"  data-toggle="modal" data-target="#emailboxpopup" style="cursor: pointer;">
                  <h2 class="display-1 text-uppercase">{$slide.title}</h2>
                  <div class="caption-description">{$slide.description nofilter}</div>
                </figcaption>
              {/if}
...
<!--<div class="custom_texthome">
  <div class="container">
    <div class="custom_text displayflex">
      <div class="item_text col-md-4">
        <div class="text_title">Acheter</div>
        <p>DES ARTICLES DE LUXE 2NDE MAIN <br>AUTHENTIFIÉS PAR DES EXPERTS</p>
      </div>
      <div class="item_text col-md-4">
        <div class="text_title">Vendre</div>
        <p>VOS ARTICLES CLÉS EN MAIN <br>EN ACHAT FERME OU EN DÉPOT</p>
      </div>
      <div class="item_text col-md-4">
        <div class="text_title">Livraison Express</div>
        <p>PAR COURSIER EN 2H (PARIS &amp; PETITE <br>COURONNE) OU EN 48H PAR DHL</p>
      </div>
    </div>
  </div>
</div>-->
