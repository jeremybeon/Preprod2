...
<table class="product" width="100%" cellpadding="4" cellspacing="0">
	...
	<tr>
		...
		{if isset($layout.before_discount)}
			<th class="product header small" width="{$layout.unit_price_tax_excl.width}%">{l s='Base price' d='Shop.Pdf' pdf='true'}</th>
		{/if}
		...
		<tr class="product {$bgcolor_class}">
			...
			<td class="product left">
				...
				{else}
					{$order_detail.product_name}
				{/if}
                                {if $order_detail.f_options}
                                    <br>{$order_detail.f_options}
				{/if}
                                <br>
                                {$order_detail.product_description_short|strip_tags}
			</td>
			<td class="product center">
