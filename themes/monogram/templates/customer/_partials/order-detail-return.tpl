      ...
      <table id="order-products" class="table table-bordered return">
        <thead class="thead-default">
          <tr>
			...
			<th>{l s='Action' d='Shop.Theme.Catalog'}</th>
          </tr>
        </thead>
        {foreach from=$order.products item=product name=products}
          <tr>
            ...
            <td class="qty text-xs-center">
              {if !$product.customizations} 
                {if $product.quantity > 1 && $product.quantity > $product.qty_returned}
					<div class="current">
					  {$product.quantity}
					</div>
				  ...
				  {else}
					{$product.quantity}
					<input type="hidden" value="1" name="order_qte_input[{$product.id_order_detail}]"  id="_desktop_return_qty_{$product.id_order_detail}" >
				  {/if}
              {else}
                {foreach $product.customizations as $customization}
                  <div class="current">
			...
			<td class="text-xs-right"><a class="displayflex boutonstyle" href="{$link->getModuleLink('oui_deposit','description', [id_product => $product.id_product])}" style="padding: 3px;font-size: 1em;line-height: 1em;">{l s='Revendre' d='Shop.Theme.Actions'}</a></td>
          </tr>
        {/foreach}
        <tfoot>
