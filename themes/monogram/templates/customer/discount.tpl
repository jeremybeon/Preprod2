...
{block name='page_content'}
  {if $cart_rules}
    ...
    <div class="mes_bondereductions">
      <div class="custom_accountpage">
        <div class="pagetitle"> 
          <h1 class="page_title">
              {l s='Mes bons de réduction' d='Shop.Theme.Customeraccount'}
            </h1>
        </div>
            ...
            <div class="reduction_elt displayflex">
              <div class="bon_desc">
                {if !empty($cart_rule.code)}
                  - {$cart_rule.value} {$cart_rule.name} avec le code {$cart_rule.code}.
                {else}
                  {$cart_rule.name}
                {/if}
                {*-10% sur tous les articles de la marque CHANEL avec le code CHANEL10.*}
              </div>
              <div class="bon_date">{l s="Valable jusqu'au" d='Shop.Theme.Customeraccount'} <b>{Tools::displayDate($cart_rule.date_to)}</b></div>
            </div>
          {/foreach}
